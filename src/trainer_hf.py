...
class Trainer:
    ...
    def _inner_training_loop(
        ...
        metrics["train_forward_time"] = avg_forward_time
        metrics["train_backward_time"] = avg_backward_time
    ...
    def evaluation_loop(
        ...
        for step, inputs in enumerate(dataloader):
            ...
            # Prediction step
            loss, logits, labels, forward_time = self.prediction_step(model, inputs, prediction_loss_only, ignore_keys=ignore_keys)
            forward_times.append(forward_time)
            main_input_name = getattr(self.model, "main_input_name", "input_ids")
            inputs_decode = self._prepare_input(inputs[main_input_name]) if args.include_inputs_for_metrics else None
            if hasattr(model, "model"):
                if hasattr(model.model, 'save_indicators'):
                    if model.model.save_indicators  and metric_key_prefix=="predict":
                        output_path = os.path.join(model.model.output_dir, 'indicators.jsonl')
                ...
                if hasattr(model.model, 'save_scores'):
                    if model.model.save_scores and metric_key_prefix=="predict":
                        output_path = os.path.join(model.model.output_dir, 'scores.jsonl')
                        with open(output_path, 'a+') as file:
        ...
        metrics[f"{metric_key_prefix}_forward_time"] = avg_forward_time
    ...
    def prediction_step(
        ...
        forward_time = forward_end_time - forward_start_time
        return (loss, logits, labels, forward_time)
    ...
    def create_accelerator_and_postprocess(self):
        ...
        del accelerator_kwargs["non_blocking"]
        del accelerator_kwargs["gradient_accumulation_kwargs"]
        del accelerator_kwargs["dispatch_batches"]
        del accelerator_kwargs["even_batches"]
